...
@include('agent.modal.peringatan')
<!-- Start of Selection -->
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-6 col-md-8">
                    <h5 class="card-title">Data Daftar Agent</h5>
                </div>
            </div>
            <div class="table-responsive mt-3">
                ...
                <table id="example" class="table table-striped table-bordered w-100 mt-3">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>{{ Str::upper($dataItem->nama_agent ?? '-') }}</td>
                            <td>{{ Str::upper($dataItem->clinic->nama_klinik ?? '-') }}</td>
                            <td>{{ Str::upper($dataItem->customer->nama_perusahaan ?? '-') }}</td>
                            <td>{{ Str::upper($dataItem->create_by ?? '-') }}</td>
                            <td>{{ Str::upper($dataItem->updated_at ?? '-') }}</td>
                            @if(Auth::user()->role == '2')
                            <td>
                                <div class="d-flex gap-1">
                                    <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#editagent{{ $dataItem->id }}">
                                        Edit
                                    </button>
                                    <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteagent{{ $dataItem->id }}">
                                        Delete
                                    </button>
                                </div>
                            </td>
...
@include('agent.modal.add')
@include('agent.modal.edit')
