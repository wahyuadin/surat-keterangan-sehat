        ...
        <div class="card-body">
            <div class="row">
                <div class="col-6 col-md-8">
                    <h5 class="card-title">Data Daftar paramedis</h5>
                </div>
            </div>
            <div class="table-responsive mt-3">
                <div class="mt-3 mb-4">
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addParamedis">
                ...
                <table id="example" class="table table-striped table-bordered w-100 mt-3">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>{{ Str::upper($dataItem->nama ?? '-') }}</td>
                            <td>{{ Str::upper($dataItem->clinic->nama_klinik ?? '-') }}</td>
                            <td>{{ Str::upper($dataItem->bagian ?? '-') }}</td>
                            <td>
                                @if($dataItem->ttd)
                                <img src="{{ asset('storage/' . $dataItem->ttd) }}" alt="TTD" style="max-width: 50px;">
                                @else
                                No Signature
                                @endif
                            </td>
                            ...
                            <td>
                                ...
                                <div class="d-flex gap-1">
                                    <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#editParamedis{{ $dataItem->id }}">
                                        Edit
                                    </button>
                                    <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteParamedis{{ $dataItem->id }}">
                                        Delete
                                    </button>
                                </div>
                                @else
                                @if(Auth::user()->clinic_id == $dataItem->clinic_id)
                                <div class="d-flex gap-1">
                                    <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#editParamedis{{ $dataItem->id }}">
                                        Edit
                                    </button>
...
@include('paramedis.modal.add')
@include('paramedis.modal.edit')
@include('paramedis.modal.delete')
@push('style')
{{-- datatable --}}
