...
@include('perusahaan.modal.peringatan')
<!-- Start of Selection -->
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-12 col-md-8">
                    <h5 class="card-title">Data Daftar Perusahaan</h5>
                </div>
            </div>
            <div class="table-responsive mt-3">
                <div class="mt-3 mb-4">
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addPerusahaan">
                ...
                <table id="example" class="table table-striped table-bordered w-100 mt-3">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ Str::upper($dataItem->nama_perusahaan ?? '-') }}</td>
                            <td>{{ Str::upper($dataItem->alamat ?? '-') }}</td>
                            <td>Rp. {{ number_format($dataItem->tarif, 0, ',', '.') }}</td>
                            @php
                            ...
                            <td>
                                <div class="d-flex gap-1">
                                    <button type="button" class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#editPerusahaan{{ $dataItem->id }}">
                                        Edit
                                    </button>
                                    @if(Auth::user()->role == '2')
                                    <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deletePerusahaan{{ $dataItem->id }}">
                                        Delete
                                    </button>
                                    @endif
                                </div>
...
@include('perusahaan.modal.add')
@include('perusahaan.modal.edit')
@include('perusahaan.modal.delete')
@push('style')
{{-- datatable --}}
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function formatCurrency(input, blur) {
            // appends $ to value, validates decimal side is only 2 digits
            // get input value
            ...
            if (input_val.indexOf(".") >= 0) {
                ...
                // validate that only 2 digits are after the decimal point
                ...
                // update input value
                input.val(left_side + right_side);
            ...
            if (blur === "blur") {
                input.prop("selectionStart", original_len);
                input.prop("selectionEnd", original_len);
            } else {
                input.prop("selectionStart", caret_pos);
                input.prop("selectionEnd", caret_pos);
            }
        }
    });
