        ...
        <div class="card-body">
            <div class="row">
                <div class="col-6 col-md-8">
                    <h5 class="card-title">Data Form Patient</h5>
                </div>
            </div>
            <div class="table-responsive mt-3">
                <div class="mt-3 mb-4">
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addPatient-template">
                ...
                <table id="example" class="table table-striped table-bordered w-100 mt-3">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>{{ Str::upper($dataItem->title ?? '-') }}</td>
                            <td>{{ Str::upper($dataItem->description ?? '-') }}</td>
                            <td>{{ $dataItem->email ?? '-' }}</td>
                            <td>
                                <a href="{{ url('/form/' . $dataItem->slug) }}" target="_blank">{{ url('/patient-form/' . $dataItem->slug) }}</a>
                            </td>
                            <td>{{ Str::upper($dataItem->clinic->nama_klinik ?? '-') }}</td>
                            @php
                            $qrData = base64_encode(
                            QrCode::format('svg')->size(300)->generate(url('/form/' . $dataItem->slug))
                            );
                            @endphp

                            <td>
                                <a href="data:image/svg+xml;base64,{{ $qrData }}" download="qr-{{ $dataItem->slug }}.svg" style="cursor: pointer;">
                                    {!! QrCode::format('svg')->size(120)->generate(url('/form/' . $dataItem->slug)) !!}
                                </a>
                            </td>
                            <td>
                                <span class="badge {{ $dataItem->is_active == 1 ? 'bg-success' : 'bg-danger' }}">
                                    {{ $dataItem->is_active == 1 ? 'ACTIVE' : 'INACTIVE' }}
                                </span>
                            </td>

                            <td>{{ Str::upper($dataItem->created_by ?? '-') }}</td>
                            <td>{{ Str::upper($dataItem->updated_at ?? '-') }}</td>
                            <td>
                                <div class="d-flex gap-1">
                                    <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#editPatient-template{{ $dataItem->id }}">
                                        Edit
                                    </button>
                                    @if(Auth::user()->role == '2')
                                    <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deletePatient-template{{ $dataItem->id }}">
                                        Delete
                                    </button>
                                    @endif
                                </div>
...
@include('patient-template.modal.add')
@include('patient-template.modal.edit')
@include('patient-template.modal.delete')
@push('style')
{{-- datatable --}}
